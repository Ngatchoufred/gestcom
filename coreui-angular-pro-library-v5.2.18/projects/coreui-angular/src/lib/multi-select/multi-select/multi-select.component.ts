...
export const observeReferenceResizeModifier: Modifier<string, Options> = {
  ...
  effect: ({ state, instance }: ModifierArguments<Options>) => {
    ...
    resizeObserver.observe(<Element>reference);
    return () => {
...
export const sameAsReferenceWidthModifier: Modifier<string, Options> = {
...
export const defaultPopperOptions: Partial<Options> = {
  strategy: 'fixed',
  modifiers: [
...
export class MultiSelectComponent<TValue extends string | number>
  implements IMultiSelect<TValue>, ControlValueAccessor, AfterViewInit, OnChanges, OnDestroy, OnInit
{
  ...
  readonly minWidth = signal(this.itemMinWidth);
  ...
  set popperOptions(options: Partial<Options>) {
    ...
    const currOptions: Partial<Options> = {
      ...options,
      modifiers: [
        ...prevModifiers.filter(
          (prevModifier) => !currModifiers.find((currModifier) => currModifier.name === prevModifier.name)
        ),
        ...currModifiers
      ]
    };
  ...
  readonly #popperOptions = signal(defaultPopperOptions);
  ...
  @ContentChildren(MultiSelectOptionComponent, { descendants: true })
  ...
  @ViewChild(DropdownMenuDirective, { read: ElementRef }) dropdownMenu!: ElementRef;
  ...
  @HostListener('keyup', ['$event']) onKeyUp($event: KeyboardEvent): void {
    ...
    if (
      ['Enter', 'Space', 'ArrowDown'].includes($event.code) &&
    ...
    if (['Tab'].includes($event.key)) {
      // todo focus on input when tab in
  ...
  updateNativeSelect() {
    ...
    this.nativeSelectRef.setInput('multiple', this.multiple);
    this.nativeSelectRef.setInput('form', this.nativeFormId);
    this.nativeSelectRef.setInput('id', this.nativeId);
    this.nativeSelectRef.setInput('name', this.nativeName);
    this.nativeSelectRef.setInput('options', [...options]);
    this.nativeSelectRef.setInput('disabled', this.disabled);
  ...
  private setFocusMonitor(monitor: boolean) {
    if (monitor && !this.disabled) {
      ...
      //   .pipe(
      //     tap(origin => {
      //       if (origin === 'keyboard' && this.search && this.selectedOptions.length) {
  ...
  get firstFocusableItem() {
    return (
      this.multiSelectOptionsContent?.find((option) => option.visible && !option.disabled) ??
