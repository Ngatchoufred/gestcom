...
export class TimePickerComponent implements OnChanges, OnInit, ControlValueAccessor, AfterViewInit {
   ...
   * @default true
   */
  @Input({ transform: booleanAttribute }) cleaner: boolean = true;
   ...
   * @default false
   */
  @Input({ transform: booleanAttribute }) seconds: boolean = false;
  ...
  getListOfHours12(): ITimeValue[] {
    const listOfHours12 = this.hour12
      ? this.listOfHours?.filter((item) => (item.dayPeriod && item.dayPeriod === this.dayPeriod) || !item.dayPeriod)
      : this.listOfHours;
    if (this.hour12) {
      listOfHours12.push(<ITimeValue>listOfHours12.shift());
    }
    return listOfHours12;
  }
  ...
  get timePickerClasses() {
    return {
      'time-picker': true,
      [`time-picker-${this.size}`]: !!this.size,
  ...
  set dayPeriod(value) {
    if (this._dayPeriod !== value) {
      ...
      if (this.listOfHours) {
        this.listOfHours12 = this.getListOfHours12();
      }
  ...
  listOfTimeSegments(
    getListOfTimeSegments: (locale: string, dateTimeFormatOptions: DateTimeFormatOptions) => ITimeValue[],
    filterTimeSegments: ((timeSegmentValue: number) => boolean) | undefined
  ) {
    const list = getListOfTimeSegments(this.locale, this.dateTimeFormatOptions).filter((segment: ITimeValue) => {
      return typeof filterTimeSegments === 'function' ? filterTimeSegments(segment.value) : true;
    });
    return list.length ? list : getListOfTimeSegments(this.locale, this.dateTimeFormatOptions).slice(0, 1);
  }
  ...
  handleSelectTimeChange(value: number, segment: 'hour' | 'minute' | 'second') {
    this[segment] = value;
    if (value >= 0) {
      this.setTime();
    }
  ...
  patchSelectTimeValues() {
    ...
    const goUpdate = (this.second < 0 && this.minute < 0) || (this.hour >= 0 && this.minute >= 0 && this.second >= 0);
  ...
  setChangeHandlers() {
    ...
    this.selectTime
      ...
      ?.valueChanges.pipe(
        tap((value) => {
          this.handleSelectTimeChange(<number>value, 'hour');
        }),
    ...
    this.selectTime
      ...
      ?.valueChanges.pipe(
        tap((value) => {
          this.handleSelectTimeChange(<number>value, 'minute');
        }),
        takeUntilDestroyed(this.#destroyRef)
      )
      .subscribe();
    ...
    this.selectTime
      ...
      ?.valueChanges.pipe(
        filter(() => this.seconds),
        tap((value) => {
          this.handleSelectTimeChange(<number>value, 'second');
        }),
        takeUntilDestroyed(this.#destroyRef)
      )
      .subscribe();
